...
(() => {
 ...
 function select(i,focus=false){const previous=selected;selected=i;transition?.cancel();figure.dataset.view=views[i][0];views.forEach((v,j)=>{const b=tabs.children[j];b.setAttribute('aria-selected',String(i===j));b.tabIndex=i===j?0:-1;});img.width=1320;img.height=[645,194,474,1400][i];img.src=new URL(i===3?'infrastructure-lab-series-animated.svg':'infrastructure-lab-'+views[i][0]+'.svg',base).href;img.alt=views[i][2];panel.setAttribute('aria-labelledby','infra-tab-'+views[i][0]);caption.textContent=views[i][2]+(i<2?' 거누의 이동은 흐름 설명용입니다.':'');if(previous!==-1&&previous!==i)transition=panel.animate([{opacity:0,transform:'translateY(7px)'},{opacity:1,transform:'translateY(0)'}],{duration:260,easing:'cubic-bezier(.22,.61,.36,1)'});if(focus)tabs.children[i].focus();}
